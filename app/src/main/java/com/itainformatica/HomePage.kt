...
fun HomePage(context: Context) {
    ...
    var viewProductList by remember { mutableStateOf(JsonData.products) }
    ...
    Column(
        ...
        SearchField(
            onSearch = {
                onSearch = it.isNotEmpty()

                viewProductList = JsonData.products
                if(onSearch){
                    viewProductList = JsonData.products.filter { item ->
        ...
        )
        {
            ...
            items(count = viewProductList.size){ index ->
                val item = viewProductList[index]
